...
**Goal:** Replace the Orion task tree with locally numbered composite directories and leaf Markdown files, and make orchestrated minimal-delta execution mandatory.

**Architecture:** The filesystem is the only child-order source: numbered directories contain composite `TASK.md` descriptions and numbered Markdown files are executable leaves. The task runner selects and plans work, while `orion-review-orchestrator` exclusively executes selected leaves through workers that apply `minimal-delta`.
...
without making both `orion-review-orchestrator` and `minimal-delta` mandatory.
...
`minimal-delta` before and during implementation. Preserve the existing review
gate, worktree isolation, user integration gate, verification ownership, and
cleanup rules.
...
requires the worker to apply `minimal-delta`, and deletes the leaf file only in
the reviewed completion commit.

**Step 5: Review for architectural duplication**

Apply `architecture-simplifier` read-only to the resulting task workflow.
