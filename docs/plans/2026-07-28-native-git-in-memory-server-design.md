...
Netty Channel
      |
GitNativeProtocolAdapter   (Netty handler — interprets Yield, manages writes)
      |
ContinuationRuntime<ByteBuf>
      |
GitServerWireMachine       (Continuation<ByteBuf> — owns all protocol phases)
      |
...
**The entire continuation hierarchy runs on a single `ContinuationRuntime<ByteBuf>`.**
All phases — pkt-line parsing, pack streaming, response generation — are
`Continuation<ByteBuf>` and receive raw `ByteBuf` chunks directly. There is no
parallel event union type, no secondary input channel, and no `written()`
callback entering the machine.

`GitServerWireMachine` implements `Continuation<ByteBuf>`. It composes
`GitMinimalWireMachine` (also `Continuation<ByteBuf>`) for pkt-line framing
and drives all protocol phases: initial request, repository resolution,
advertisement, service request, pack transfer, response, and terminal.

When the machine needs to send a response before reading more data, it enqueues
the outbound buffer into an `OutputSink` and returns `ContinuationFlow.Yield`.
The runtime suspends. The handler intercepts `runtime.pendingTask()`, writes the
buffer, and calls `runtime.resumeTask()` when the write completes — dispatching
back to the event loop thread if needed. Back-pressure cascades naturally: if
the channel is not writable the handler waits for `channelWritabilityChanged`
before calling `resumeTask`.

`GitNativeProtocolAdapter` is a Netty bridge only. It forwards inbound
`ByteBuf`s to `runtime.accept()`, intercepts `Yield` to schedule writes, and
calls `runtime.resumeTask()` on write completion. It does not parse Git messages
or keep a second protocol phase.

### Handler without Netty

Because the machine is a plain `Continuation<ByteBuf>` and `ContinuationRuntime`
is scheduler-agnostic, a test or embedded handler can drive the machine without
Netty:

```java
ContinuationRuntime<ByteBuf> runtime = new SimpleRuntime(machine);

runtime.accept(inputChunk);
while (runtime.isYielding()) {
    runtime.pendingTask().run();  // flush OutputSink synchronously
    runtime.resumeTask();         // re-enter machine
}
```

This is sufficient to exercise all protocol phases in unit tests without
`EmbeddedChannel`.
