...
### Task 1: Add Application Option Coverage

**Files:**
- Modify: `core/bootstrap/src/test/java/pro/deta/orion/AppOptionsTest.java`
- Add: `core/bootstrap/src/test/java/pro/deta/orion/ReleaseVerifierTest.java`
...
- normal application options continue to parse through `AppOptions`;
- `run` forwards remaining application options;
- `verify` options parse through `AppOptions`;
...
```sh
mvn test -Pdev -pl core/bootstrap -am -Dtest=AppOptionsTest,ReleaseVerifierTest,ExecutableLauncherScriptTest,AppTest -Dsurefire.failIfNoSpecifiedTests=false
```

Expected: fails before implementation because verify options and verifier behavior are missing.

### Task 2: Extend `AppOptions` and Add Verifier
...
- Add: `core/bootstrap/src/main/java/pro/deta/orion/ReleaseVerifier.java`

**Step 1: Add command dispatch**

`App.main` should parse all command line input through `AppOptions`. `AppOptions` should support:
