...
The integrated pack-index lookup foundation defines Git pack index
...
The integrated thin-pack generation foundation defines outbound
...
[Externalized repository storage](06_externalized-repository-storage.md) needs S3 range
reads, pack index lookup, metadata batching, and eventually reusing existing
deltas and pack slices for upload-pack.

[The reachability acceleration task](11_reachability-acceleration-indexes.md) defines pack
locality summaries as a future optimization input for pack planning and delta
base selection.

[Repository maintenance](09_repository-maintenance.md) needs repack and
locality improvements after safe liveness and pruning exist.

The integrated native upload-pack path can initially build
