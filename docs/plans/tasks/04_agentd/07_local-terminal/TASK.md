...
The launch-only slice is complete in `e3822a4a`; the broader design governs the
remaining interactive follow-up.

Provide a local AgentD path for starting and later attaching to an independent
`session-host` without an Orion server.

The remaining child adds interactive attachment, journal rendering, input,
resize, and manual control while retaining its broader dependencies.
