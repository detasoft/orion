...
`COMMAND_RESULT` stores the exact opaque server command item as a CBOR byte
string and the operation outcome. Result outcomes are succeeded `1`, failed `2`,
...
request has no operation sequence. Schema-2 operation controls use the
...
| `0x0007` | `ACK_JOURNAL` | v2 operation wrapper and journal event ID |
...
The frame sequence is nonzero and is not `u64::MAX`. The command envelope is
a nonempty, opaque, exact server CBOR item. It may contain the server command
identifier, but that identifier is not duplicated in the native operation
wrapper. The host uses only the operation sequence for the high-water mark;
the envelope remains opaque. The 16 MiB maximum
applies to the complete wrapped payload. `control-idempotency-v2.bin` freezes
one request for each operation control, including `ACK_JOURNAL`, unsigned
values above `i64::MAX`, and unknown envelope fields.
The older schema-1 operation fixture remains frozen for byte compatibility,
but the live host rejects schema 1 for these five operation controls.
