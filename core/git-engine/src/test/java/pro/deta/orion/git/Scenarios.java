...
import pro.deta.orion.util.stream.ClientIO;
...
public class Scenarios {
    public static BiConsumer<Repository, SoftAssertions> fetchEmptyRepo = (r, sa) -> {
        IoConsumer<ClientIO> testStateClient = new AssertiveIOClient("""
                    ...
                    """, sa);
        try {
            testPipeScenario(testStateClient, (serverIO -> {
    ...
    public static BiConsumer<Repository, SoftAssertions> simplePatchToReceive = (r, sa) -> {
    ...
    public static IoConsumer<ServerIO> uploadPack(Repository r) {
        return serverIO -> {
            UploadPack uploadPack = GitUtils.uploadPack(r, Set.of("version=2"));
            GitUtils.upload(uploadPack, serverIO.ioEStreams());
        };
    ...
    public static IoConsumer<ServerIO> receivePack(Repository r) {
        return serverIO -> {
