...
final class TerminalCellWidth {
    ...
    static int width(String cluster) {
        ...
        for (int offset = 0; offset < cluster.length();) {
            ...
            if (point == 0xFE0F || point == 0x20E3 || point >= 0x1F1E6 && point <= 0x1F1FF) {
                width = 2;
                continue;
            }
            ...
            width = Math.max(width, wide(point) ? 2 : 1);
        }
        return width;
    }

    private static boolean wide(int point) {
        int low = 0;
        int high = WIDE.length / 2 - 1;
        while (low <= high) {
            int middle = (low + high) >>> 1;
            if (point < WIDE[middle * 2]) {
                high = middle - 1;
            } else if (point > WIDE[middle * 2 + 1]) {
                low = middle + 1;
            } else {
                return true;
            }
