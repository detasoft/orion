...
class TimedContinuationRuntimeTest {
    ...
    @Test
    void resumeTaskRestartsIdleTimeout() {
        MutableTicker ticker = new MutableTicker(0);
        YieldThenAwait continuation = new YieldThenAwait();
        TestTimedRuntime runtime = new TestTimedRuntime(continuation, ticker);

        ticker.time = 5;
        RuntimeFlow flow = runtime.accept(new Input("a"));

        ticker.time = 14;
        ((ContinuationFlow.Yield<Input>) flow).task().run();
        runtime.resumeTask();

        assertThat(runtime.lastInputTime()).isEqualTo(14);
    ...
    private static final class YieldThenAwait implements Timed, Continuation<Input> {
